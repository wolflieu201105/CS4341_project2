...
def makeMove(board, turns, blue):
    contents = "Please return the asnswer in 4.5 seconds"
    contents += "This is a game called larsker morris, I want you to provide the best move possible. Here is how I symbolize the positions: The board has the following format:\n"
    contents += "a7\t\t\td7\t\t\tg7\n"
    contents += "\tb6\t\td6\t\tf6\t\n"
    contents += "\t\tc5\td5\te5\t\t\n"
    contents += "a4\tb4\tc4\t\te4\tf4\tg4\n"
    contents += "\t\tc3\td3\te3\t\t\n"
    contents += "\tb2\t\td2\t\tf2\t\n"
    contents += "a1\t\t\td1\t\t\tg1\n"
    contents += "you will win the game when you make the opponent doesn't have any move left or the total number of mills on hand and on board equals to 2.\n"
    contents += "you will have a mill when the move you just made create a 3 consecutive positions of your man vertically or horizontally. For example, a4 b4 c4, b2 b4 b6 are valid mills while a7 b6 c5 and b4 c4 e4 are not valid mills. When a mill is created, you MUST remove the opponent's man that is not in mill unless all of the opponent's man are in mill"
    contents += "try to create as much mill as possible, you can break a mill and make it back in order to create a mill again."
    contents += "In phase 1 (when you still have pieces on hand, if you are blue, decode the first part as h1, if you are orange, decode the first part as h2.) The second part would be the position of the man you want to put in, and the third part would be the position of the opponent that you want to remove if you have a mill. If you don't have a mill, the third part would be r0\n"
    contents += "In phase 2 (when you don't have any pieces on hand and you have more than 3 man), the first part would be the man that you want to move, the second part is the postion than you want your man to move to (in this phase, you can only move your man to a position next to the previous position), and again, the third part would be the position of the opponent that you want to remove if you have a mill. If you don't have a mill, the third part would be r0\n"
    contents += "In phase 3 (when you only have 3 man left), the first part would be the man that you want to move, the second part is the postion than you want your man to move to (in this phase, you can move your man to any empty spaces), and again, the third part would be the position of the opponent that you want to remove if you have a mill. If you don't have a mill, the third part would be r0\n"
    contents += "Some examples are h1 d3 r0, a7 a4 d4\n\n"
    contents += "In this game, you are "
    contents += "blue. " if blue else "orange. "
    contents += "What is the best possible move for you?\n"
    if turns <= 20:
        contents += "You have "
        if blue:
            num = (20 - turns)/2
            ...
            num = (20 - turns + 1)/2
            contents += str(num)
        contents += " pieces on hand\n"
    else:
        contents += "You don't have any pieces on hand\n"
    
    contents += "The board is as follows: (1 is your man, 0 is empty, -1 is the opponent's man\n"
    contents += printBoard(board)
    contents += "You just need to give out the move, nothing else"
